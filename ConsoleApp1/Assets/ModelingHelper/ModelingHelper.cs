...
public static class ModelingHelper
{
    ...
    public static void Generate_4_Selected(List<Vertex> SelectedVertices, ModelMesh Mesh)
    {
        Vertex A = SelectedVertices[0];
        Vertex B = SelectedVertices[1];
        Vertex C = SelectedVertices[2];
        Vertex D = SelectedVertices[3];

        // if A and B are opposite, swap B and C
        // else if A and C are neighbors, swap C and D

        if (!A.ShareEdgeWith(B))
            (B, C) = (C, B);
        else if (A.ShareEdgeWith(C))
            (C, D) = (D, C);
